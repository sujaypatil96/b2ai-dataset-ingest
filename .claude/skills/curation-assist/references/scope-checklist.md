...
| **Redundant facet** | same phenotype already captured by a sibling item — check the candidate term's **branch** before assuming redundancy | GAD-7 `trouble_relaxing` (somatic-tension facet of anxiety, already carried by `nervous_anxious`→Anxiety); ASRS's 9 inattention items → one *Short attention span*. **Not** `worry_too_much`: it maps to *Ruminations*, under **Abnormal preoccupation** — a different branch, so a distinct phenotype rather than a facet of Anxiety |
| **Bidirectional / no clean term** | "X **or** its opposite" with no single HPO node, **or** a construct HPO does not cover at all | PHQ-9 `move_speak_slow` ("slow **or** restless"): HPO has no depressive psychomotor-retardation term — searched "psychomotor retardation"/"psychomotor", which return only global-developmental-delay terms, and HP:0002361 *Psychomotor deterioration* is "loss of previously present abilities" (neurodegeneration), not two weeks of slowing. GAD-7 `cant_control_worry`: the item is the *uncontrollability* of worry; HP:5200233 *Anticipatory anxiety* is future-orientation, a different construct |
